 ...
 * @since 1.0.0
 */
class xevso_team2_Widget extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'xevso Team Two', 'xevsocore' );
	}
	...
	protected function _register_controls() {
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => 'yes',
                'condition' => [
                    'show_team_two_enable_slide' => 'yes',
                ]
            ]
        );
        $this->add_control(
            'xevso_team_two_aplay',
            [
        ...
        $this->add_control(
            ...
            [
                ...
                'condition' => array(
                    'show_team_two_enable_slide' => 'yes',
                    'xevso_team_two_aspeed_enable' => 'yes',

                )
            ]
        );
        $this->add_control(
            'xevso_team_two_speed',
            [
        ...
        $this->add_group_control(
            ...
            [
                'name' => 'xevso_team_two_title_typo',
        ...
        $this->add_group_control(
            Group_Control_Background::get_type(),
            [
                'name' => 'xevso_team_two_siconbox_bd',
	...
	protected function render() {
        ...
        if($settings['show_team_two_enable_slide'] == 'yes' ){
            if($settings['xevso_team_two_aplay'] == 'yes' ){
                $aplay = 'true';
            }else{
                $aplay = 'false';
            }
            if($settings['xevso_team_two_nav'] == 'yes' ){
                $nav = 'true';
            }else{
                $nav = 'false';
            }
			...
			jQuery(document).ready(function($) {
				...
				$("#team-'.esc_attr($dynamic_num).'").slick({
					...
					slidesToShow:3,
					slidesToScroll:1,';
                  if(!empty($settings['xevso_team_two_aspeed'])){
                        echo 'autoplaySpeed:'.esc_attr($settings['xevso_team_two_aspeed']).',';
                    }
                    if(!empty($settings['xevso_team_two_speed'])){
                        echo 'speed:'.esc_attr($settings['xevso_team_two_speed']).',';
                    }
            echo '
					responsive: [
						{
		...
		?>
		<div class="team-boxs team-two">
			<div class="team-box">
                <div class="team-items row" id="team-<?php echo esc_attr($dynamic_num); ?>">
                 <?php  global $post;
                 $p = new \WP_Query(array('posts_per_page' => $settings['xevso_team_two_showitems'], 'post_type' => 'team' ));
                 while($p->have_posts()) : $p->the_post();
                    $xevso_idd = get_the_ID();
                    $xevso_team_two_meta = get_post_meta($xevso_idd, 'xevso_teammeta', true);
                    ?>
                    <div class="item <?php if( empty( $settings['show_team_two_enable_slide'] == 'yes' ) ) : ?>col-12 col-md-3 col-lg-3 col-xl-3 no-slide<?php endif; ?>">
                        <div class="team-single">
                            <div class="team-image">
                                <?php the_post_thumbnail(); ?>
                                <div class="team-social">
                                    <ul>
                                        <?php foreach($xevso_team_two_meta['xevso_team_socials'] as $xevso_team_two_social ) : ?>
                                        <li><a data-toggle="tooltip" data-placement="right" data-original-title="<?php echo esc_attr($xevso_team_two_social['xevso_team_social_label']) ?>" href="<?php echo esc_url($xevso_team_two_social['xevso_team_social_url']) ?>"><span class="<?php echo esc_attr($xevso_team_two_social['xevso_team_social_icon']) ?>"></span></a></li>
                                        <?php endforeach ?>
                                    </ul>
                                </div>
                            </div>
                            <div class="team-content">
                                ...
                                <?php if(!empty($xevso_team_two_meta['xevso_team_stitle'])) : ?>
                                <h4><?php echo esc_html($xevso_team_two_meta['xevso_team_stitle']) ?></h4>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
...
Plugin::instance()->widgets_manager->register_widget_type( new xevso_team2_Widget );