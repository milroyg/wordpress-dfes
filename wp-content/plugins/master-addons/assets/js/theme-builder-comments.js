...
(function($) {
	...
	jQuery(document).ready(function($) {
		...
		function like_dislike_Cookies(trigger_type, comment_id, $this, not_trigger_type) {
			...
			$.ajax({
				...
				success: function(res) {
					...
					jltma_deleteCookie("jltma_" + not_trigger_type + "_" + comment_id);
				...
				if (!$this.hasClass("jltma-dislike-trigger")) {
					...
					if (jltma_dislike_cookie != "") {
						...
						if (end_character == "K" || end_character == "M" || end_character == "B" || end_character == "T") {
							...
							if (new_dislike_count < 0) new_dislike_count = 0;
							var new_like_count = parseInt(current_like_count) + 1;
						}
					}
				...
				if (!$this.hasClass("jltma-like-trigger")) {
					...
					if (jltma_like_cookie != "") {
						...
						if (end_character == "K" || end_character == "M" || end_character == "B" || end_character == "T") {
							...
							var new_like_count = parseInt(current_like_count) - 1;
							if (new_like_count < 0) new_like_count = 0;
							var new_dislike_count = parseInt(current_dislike_count) + 1;
						}
					}
					$("#jltma-like-count-" + comment_id).html(new_like_count);
		...
		$(".jltma-comments-wrap").on("click", ".jltma-page-link", function(e) {
			...
			var post_id = $("#jltma-current-post-id").val(), page_number = $(this).data("page-number"), total_page = $(this).data("total-page"), pagination_type = $(this).data("pagination-type"), template = $(this).closest(".jltma-comments-wrap").data("jltma-comment-settings").template;
			$.ajax({
				type: "post",
				url: jltma_localize_comments_data.ajax_url,
				data: {
				...
				success: function(res) {
					...
					$("html, body").animate({ scrollTop: $(".jltma-comment-listing-wrap").offset().top }, 1e3);
				}
			});
		});
		$("body").on("click", ".jltma-next-page,.jltma-previous-page", function() {
			var post_id = $("#jltma-current-post-id").val(), total_page = $(this).data("total-page"), pagination_type = $(this).data("pagination-type"), current_page = $(this).closest(".jltma-comment-pagination-wrapper").find(".jltma-current-page").data("page-number"), next_page = parseInt(current_page) + 1, previous_page = parseInt(current_page) - 1, template = $(this).closest(".jltma-comments-wrap").data("jltma-comment-settings").template;
			if ($(this).hasClass("jltma-previous-page")) current_page = previous_page;
			else current_page = next_page;
			$.ajax({
				type: "post",
				url: jltma_localize_comments_data.ajax_url,
				data: {
		...
		function pagination_html_function(page_number, total_page, pagination_type) {
			...
			if (current_page > 1) pagination_html += "<li class=\"jltma-previous-page-wrap\"><a href=\"javascript:void(0);\" class=\"jltma-previous-page\" data-total-page=\"" + total_page + "\" data-pagination-type=\"page-number\"><i class=\"fa fa-angle-left\"></i></a></li>";
			...
			$.ajax({
				...
				success: function(data) {
					if (data) {
						...
						if (jltma_localize_comments_data.jc_page == 1) button.remove();
					} else button.remove();
				}
			});
			return false;
