( function () {
	...
	function prepareMethod( btn ) {
		...
		fetch( data.ajaxUrl, {
			...
			.then( function ( json ) {
				...
				var serverMessage = json && json.data && json.data.message ? json.data.message : '';
				if ( serverMessage ) {
					showNotice( scope, serverMessage );
					handled = true;
					return;
				}
	...
	// Cool Timeline Pro pollutes/breaks AJAX JSON on activate (plugin boot),
	// even though activation succeeds. Reload only for that slug so the Pro
	// dashboard appears. Other addons keep Activated + Check Setup Guide.
	function isCoolTimelinePro( slug ) {
		return slug === 'cool-timeline-pro';
	}

	function finishActivate( btn, labelEl, slug ) {
		if ( isCoolTimelinePro( slug ) ) {
			window.location.reload();
			return;
		}
		markActivated( btn, labelEl );
	}
	...
	function installPlugin( btn ) {
		...
		var reloadCtlPro = isActivate && isCoolTimelinePro( slug );
		...
		fetch( data.ajaxUrl, {
			...
			.then( function ( text ) {
				// CTL Pro activate: response is often unreadable / falsely "failed"
				// while the plugin did activate — always reload to show Pro UI.
				if ( reloadCtlPro ) {
					window.location.reload();
					return;
				}
				...
				var looksLikeHtml = trimmed.indexOf( '<!' ) === 0
					|| trimmed.indexOf( '<html' ) !== -1
					|| trimmed.indexOf( '<!DOCTYPE' ) !== -1;

				if ( looksLikeHtml && trimmed.length > 2000 ) {
					finishActivate( btn, labelEl, slug );
					return;
				}
				...
				var errMsg = response && response.data
					...
					: '';
				resetButton( btn, labelEl, originalLabel, errMsg || install.labels.error );
			} )
			.catch( function () {
				if ( reloadCtlPro ) {
					window.location.reload();
				...
				resetButton( btn, labelEl, originalLabel, install.labels.error );
			} );
	}

	function markActivated( btn, labelEl ) {
		btn.classList.remove( 'is-loading' );
	...
	function appendSetupGuideLink( btn ) {
		...
		link.className = 'cpo-button cpo-button-secondary cpo-setup-guide';
		link.href = setupUrl;
	...
	function resetButton( btn, labelEl, originalLabel, errorMessage ) {
		btn.classList.remove( 'is-loading' );
		btn.disabled = false;
		if ( labelEl ) {
