...
class Eac_Load_Nav_Menu {
	...
	public function update_nav_menu_title( $title, $item, $args, $depth ) {
		...
		if ( ! empty( $meta_badge ) ) {
			...
			$badge              = '<span class="nav-menu_item-badge" style="color:' . esc_attr( $menu_badge_color ) . '; background-color:' . esc_attr( $menu_badge_bgcolor ) . ';">' . esc_html( $meta_badge ) . '</span>';
		}
	 ...
	 */
	public function add_menu_item_fields( $item_id ) {
	...
	public function save_menu_settings() {
		...
		if ( isset( $fields_on['menu-item_badge'] ) && ! empty( $fields_on['menu-item_badge'] ) ) {
			$args['badge']['content'] = sanitize_text_field( $fields_on['menu-item_badge'] );
		}
		...
		if ( isset( $fields_on['menu-item_badge-color-picker'] ) && ! empty( $fields_on['menu-item_badge-color-picker'] ) ) {
			$args['badge']['color'] = sanitize_hex_color( $fields_on['menu-item_badge-color-picker'] );
		}
		...
		if ( isset( $fields_on['menu-item_badge-background-picker'] ) && ! empty( $fields_on['menu-item_badge-background-picker'] ) ) {
			$args['badge']['bgcolor'] = sanitize_hex_color( $fields_on['menu-item_badge-background-picker'] );
		}

		// Pick list des icones
		if ( isset( $fields_on['menu-item_icon-picker'] ) && ! empty( $fields_on['menu-item_icon-picker'] ) ) {
			$args['icon']   = sanitize_text_field( $fields_on['menu-item_icon-picker'] );
		}
		...
		if ( isset( $fields_on['menu-item_thumbnail-sizes'] ) ) {
			$args['thumbnail']['sizes'] = sanitize_text_field( $fields_on['menu-item_thumbnail-sizes'] );
		}

		// URL de l'image
		if ( isset( $fields_on['menu-item_image-picker'] ) && ! empty( $fields_on['menu-item_image-picker'] ) ) {
			$args['image']['url'] = esc_url_raw( sanitize_text_field( $fields_on['menu-item_image-picker'] ) );
		}

		// Dimension de l'image
		if ( isset( $fields_on['menu-item_image-sizes'] ) ) {
			$args['image']['sizes'] = sanitize_text_field( $fields_on['menu-item_image-sizes'] );
		}
