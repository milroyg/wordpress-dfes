...
class Eac_Woo_Filters {
	...
	private function __construct() {
		...
		if ( $this->options ) {
			if ( isset( $this->options['mini_cart'] ) ) {
				$this->mini_cart = $this->options['mini_cart'];
			}

			/** Ancien format de l'option */
			if ( isset( $this->options['catalog']['active'] ) ) {
				$this->catalog = $this->options['catalog']['active'];
			} elseif ( isset( $this->options['catalog'] ) ) {
				$this->catalog = $this->options['catalog'];
			}

			$this->redirect_pages = isset( $this->options['redirect_pages'] ) && $this->options['redirect_pages'];

			/** Ancien format de l'option */
			if ( isset( $this->options['product-page']['redirect_buttons'] ) ) {
				$this->redirect_buttons = $this->options['product-page']['redirect_buttons'];
			} elseif ( isset( $this->options['product-page']['redirect'] ) ) {
				$this->redirect_buttons = $this->options['product-page']['redirect'];
			}

			$this->request_quote = isset( $this->options['catalog']['request_quote'] ) && $this->options['catalog']['request_quote'];
			$this->redirect_metas = isset( $this->options['product-page']['metas'] ) && $this->options['product-page']['metas'];
			$this->breadcrumb = isset( $this->options['product-page']['breadcrumb'] ) && $this->options['product-page']['breadcrumb'];
			...
			*/
			add_action( 'woocommerce_init', array( $this, 'add_woo_hooks' ) );
		}
	}
	...
	public function add_woo_hooks() {
		...
		$this->redirect_url = ! empty( $this->options['product-page']['shop']['id'] ) ? get_permalink( $this->options['product-page']['shop']['id'] ) : '';
		...
		if ( $this->breadcrumb ) {
			add_filter( 'woocommerce_get_breadcrumb', array( $this, 'change_terms_breadcrumb_url' ), 9999, 1 );
		}
		...
		/** Transforme le site en catalogue de produits, dequeue le script fragment page */
		if ( $this->catalog ) {
			add_action( 'wp_enqueue_scripts', function () {
				wp_dequeue_script( 'wc-cart-fragments' );
			}, 11 );
			if ( $this->request_quote ) {
		...
		add_filter( 'woocommerce_sale_flash', array( $this, 'turn_catalog_mode_on_for_sale' ), 9999, 1 );
		add_filter( 'woocommerce_get_stock_html', array( $this, 'turn_catalog_mode_on_for_stock' ), 9999, 1 );
	 ...
	 */
	public function turn_catalog_mode_on_for_sale( $html ) {
	 ...
	 */
	public function turn_catalog_mode_on_for_stock( $html ) {
		if ( $this->catalog && ( is_front_page() || is_shop() || is_product() || is_product_category() || is_product_tag() ) ) {
			return '';
		} else {
			return $html;
	...
	public function change_terms_breadcrumb_url( $crumbs ) {
		$url       = $this->redirect_url;
	 ...
	 * @return bool
	 */
	public function is_catalog(): bool {
		return $this->catalog;
	}
	 ...
	 * @return void
	 */
	public function set_mini_cart( bool $state = false ): void {
		if ( $this->options ) {
			$this->mini_cart = null === filter_var( $state, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE ) ? false : $state;
			if ( isset( $this->options['mini_cart'] ) ) {
				$this->options['mini_cart'] = null === filter_var( $state, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE ) ? false : $state;
				update_option( $this->options_shop_name, $this->options, false );
			}
		}
	}
	 ...
	 * @return bool
	 */
	public function is_wc_page(): bool {
		if ( $this->options && ! empty( $this->options['product-page']['shop']['id'] ) ) {
			$id = get_the_ID() === (int) $this->options['product-page']['shop']['id'];
			if ( $id || is_shop() || is_cart() ) {
				return true;
			}
		}
		return false;
