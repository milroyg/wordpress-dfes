<?php
/**
 * Class: Eac_Acf_Repeater_Field
 * Name: EAC Repeater
 * Type: ACF field type 'eac_repeater'
 *
 * Description: Création et gestion d'un nouveau type de champ ACF 'eac_repeater'
 *
 * @since 2.4.9
 */

namespace EACCustomWidgets\Includes\Acf\Fields\Repeater;

defined( 'ABSPATH' ) || exit;

use EACCustomWidgets\EAC_Plugin;
use EACCustomWidgets\Includes\Acf\Fields\Repeater\Class_ACF_Repeater_Table;

class Eac_Acf_Repeater_Field extends \acf_field {
	...
	public function initialize() {
		$this->name          = 'eac_repeater';
		$this->label         = esc_html__( 'EAC Repeater', 'eac-components' );
		$this->category      = 'layout';
		$this->description   = esc_html__( 'Allows you to select and display existing fields. It does not duplicate any fields in the database, but loads and displays the selected fields at run-time.', 'eac-components' );
		$this->preview_image = acf_get_url() . '/assets/images/field-type-previews/field-preview-repeater.png';
		$this->doc_url       = 'https://elementor-addon-components.com/how-to-use-and-display-the-acf-repeater-field-with-elementor/';
	...
	public function input_admin_enqueue_scripts() {
		wp_register_script( 'eac-bkend-repeater', EAC_Plugin::instance()->get_script_url( 'includes/acf/fields/repeater/assets/js/input' ), array( 'jquery', 'acf-input' ), EAC_PLUGIN_VERSION, true );
		wp_enqueue_script( 'eac-bkend-repeater' );

		/**wp_register_script( 'eac-acf-repeater-collapse', EAC_Plugin::instance()->get_script_url( 'includes/acf/fields/repeater/assets/js/field-group' ), array( 'jquery', 'acf-input', 'eac-bkend-repeater' ), EAC_PLUGIN_VERSION, true );
		wp_enqueue_script( 'eac-acf-repeater-collapse' );*/

		wp_register_style( 'eac-bkend-repeater', EAC_Plugin::instance()->get_style_url( 'includes/acf/fields/repeater/assets/css/input' ), array( 'acf-input' ), EAC_PLUGIN_VERSION );
		wp_enqueue_style( 'eac-bkend-repeater' );

		acf_localize_text(
			array(
				'Minimum rows not reached ({min} rows)' => esc_html__( 'Minimum rows not reached ({min} rows)', 'eac-components' ),
				'Maximum rows reached ({max} rows)' => esc_html__( 'Maximum rows reached ({max} rows)', 'eac-components' ),
				'Error loading page'                => esc_html__( 'Error loading page', 'eac-components' ),
				'Order will be assigned upon save'  => esc_html__( 'Order will be assigned upon save', 'eac-components' ),
			)
		);
	}
	...
	public function load_field( $field ) {
		...
		if ( '' === $field['button_label'] ) {
			$field['button_label'] = __( 'Add Row', 'eac-components' );
		}
	...
	public function render_field( $field ) {
		...
		$table               = new Class_ACF_Repeater_Table( $field );
		$table->render();
	}
	...
	public function render_field_settings( $field ) {
		...
		<div class="acf-field acf-field-setting-sub_fields" data-setting="repeater" data-name="sub_fields">
			<div class="acf-label">
				<label><?php esc_html_e( 'Sub Fields', 'eac-components' ); ?></label>
		...
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'Layout', 'eac-components' ),
				'instructions' => '',
				...
				'layout'       => 'horizontal',
				'choices'      => array(
					'table' => __( 'Table', 'eac-components' ),
					'block' => __( 'Block', 'eac-components' ),
					'row'   => __( 'Row', 'eac-components' ),
				),
			)
		);
		...
		if ( $supports_pagination ) {
			acf_render_field_setting(
				...
				array(
					...
					'instructions' => __( 'Useful for fields with a large number of rows.', 'eac-components' ),
			...
			acf_render_field_setting(
				$field,
				array(
					'label'        => __( 'Rows Per Page', 'eac-components' ),
					'instructions' => __( 'Set the number of rows to be displayed on a page.', 'eac-components' ),
	...
	public function render_field_validation_settings( $field ) {
		...
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'Minimum Rows', 'eac-components' ),
				'instructions' => '',
				'type'         => 'number',
				'name'         => 'min',
				'placeholder'  => '0',
		...
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'Maximum Rows', 'eac-components' ),
				'instructions' => '',
				'type'         => 'number',
				'name'         => 'max',
				'placeholder'  => '0',
	...
	public function render_field_presentation_settings( $field ) {
		...
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'Collapsed', 'eac-components' ),
				'instructions' => __( 'Select a sub field to show when row is collapsed', 'eac-components' ),
				'type'         => 'select',
				'name'         => 'collapsed',
				'allow_null'   => 1,
				'choices'      => $choices,
		...
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'Button Label', 'eac-components' ),
				'instructions' => '',
				'type'         => 'text',
				'name'         => 'button_label',
				'placeholder'  => __( 'Add Row', 'eac-components' ),
			)
		);
	}
	...
	public function validate_value( $valid, $value, $field, $input ) {
		...
		if ( empty( $field['pagination'] ) && $min && $count < $min ) {
			// create error
			$error = __( 'Minimum rows not reached ({min} rows)', 'eac-components' );
	...
	public function validate_rest_value( $valid, $value, $field ) {
		if ( ! is_array( $value ) && is_null( $value ) ) {
			...
			$error = sprintf( __( '%s must be of type array or null.', 'eac-components' ), $param );
	...
	public function ajax_get_rows() {
		...
		if ( ! acf_verify_ajax( $args['nonce'], $args['field_key'], true ) ) {
			$error = array( 'error' => __( 'Invalid nonce.', 'eac-components' ) );
			wp_send_json_error( $error, 401 );
		}

		if ( '' === $args['field_name'] || '' === $args['field_key'] ) {
			$error = array( 'error' => __( 'Invalid field key or name.', 'eac-components' ) );
		...
		if ( ! $field || ! $post_id ) {
			$error = array( 'error' => __( 'There was an error retrieving the field.', 'eac-components' ) );
			wp_send_json_error( $error, 404 );
		}
		...
		$repeater_table   = new Class_ACF_Repeater_Table( $field );
...
add_action('init', function () {
	acf_register_field_type( 'EACCustomWidgets\Includes\Acf\Fields\Repeater\Eac_Acf_Repeater_Field' );
}, 99 );
