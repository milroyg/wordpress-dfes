<?php
/**
 * Class: Class_ACF_Repeater_Table
 * Name: EAC Repeater
...
class Class_ACF_Repeater_Table {
	 ...
	 * @since 6.0.0
	 *
	 * @param boolean $retour If we should return the rows or render them.
	 * @return array|void
	 */
	public function rows( $retour = false ) {
		$rows = array();

		// Don't include the clone when rendering via AJAX.
		if ( $retour && isset( $this->value['acfcloneindex'] ) ) {
			unset( $this->value['acfcloneindex'] );
		}

		foreach ( $this->value as $i => $row ) {
			$rows[ $i ] = $this->row( $i, $row, $retour );
		}
	 ...
	 * @param boolean $retour If we should return the row or render it.
	 * @return string|void
	 */
	public function row( $i, $row, $retour = false ) {
		if ( $retour ) {
			ob_start();
		}
	...
	public function row_handle( $i ) {
		...
		$title        = __( 'Drag to reorder', 'eac-components' );
		$row_num_html = sprintf(
			'<span class="acf-row-number" title="%s">%d</span>',
			esc_html__( 'Click to reorder', 'eac-components' ),
			$hr_row_num
		);
		...
		<td class="<?php echo esc_attr( $classes ); ?>" title="<?php echo esc_attr( $title ); ?>">
			<?php if ( $this->field['collapsed'] ) : ?>
				<a class="acf-icon -collapse small" href="#" data-event="collapse-row" title="<?php esc_attr_e( 'Click to toggle', 'eac-components' ); ?>"></a>
			<?php endif; ?>
	...
	public function row_actions() {
		...
		<td class="acf-row-handle remove">
			<a class="acf-icon -plus small acf-js-tooltip hide-on-shift" href="#" data-event="add-row" title="<?php esc_attr_e( 'Add row', 'eac-components' ); ?>"></a>
			<a class="acf-icon -duplicate small acf-js-tooltip show-on-shift" href="#" data-event="duplicate-row" title="<?php esc_attr_e( 'Duplicate row', 'eac-components' ); ?>"></a>
			<a class="acf-icon -minus small acf-js-tooltip" href="#" data-event="remove-row" title="<?php esc_attr_e( 'Remove row', 'eac-components' ); ?>"></a>
		</td>
		<?php
	}
	...
	public function pagination() {
		...
		$html_current_page = sprintf(
			...
			'<label for="current-page-selector" class="screen-reader-text">' . __( 'Current Page', 'eac-components' ) . '</label>',
			1,
			strlen( $total_pages )
		);
		...
		<div class="acf-tablenav tablenav-pages">
			<a class="first-page button acf-nav" aria-hidden="true" data-event="first-page" title="<?php esc_attr_e( 'First Page', 'eac-components' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'First Page', 'eac-components' ); ?></span>
				<span aria-hidden="true">&laquo;</span>
			</a>
			<a class="prev-page button acf-nav" aria-hidden="true" data-event="prev-page" title="<?php esc_attr_e( 'Previous Page', 'eac-components' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Previous Page', 'eac-components' ); ?></span>
				<span aria-hidden="true">&lsaquo;</span>
			</a>
			<span class="paging-input">
				<label for="current-page-selector" class="screen-reader-text"><?php esc_html_e( 'Current Page', 'eac-components' ); ?></label>
				<span class="tablenav-paging-text" title="<?php esc_attr_e( 'Current Page', 'eac-components' ); ?>">
				<?php
				printf(
					/* translators: 1: Current page, 2: Total pages. */
					esc_html_x( '%1$s of %2$s', 'paging', 'eac-components' ),
			...
			<a class="next-page button acf-nav" data-event="next-page" title="<?php esc_attr_e( 'Next Page', 'eac-components' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Next Page', 'eac-components' ); ?></span>
				<span aria-hidden="true">&rsaquo;</span>
			</a>
			<a class="last-page button acf-nav" data-event="last-page" title="<?php esc_attr_e( 'Last Page', 'eac-components' ); ?>">
				<span class="screen-reader-text"><?php esc_html_e( 'Last Page', 'eac-components' ); ?></span>
