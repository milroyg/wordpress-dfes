...
namespace EACCustomWidgets\Includes\Acf\Fields\Gallery;

defined( 'ABSPATH' ) || exit;

use EACCustomWidgets\EAC_Plugin;

class Eac_Acf_Gallery_Field extends \acf_field {
	/**
	...
	public function __construct() {
		...
		$this->label         = esc_html__( 'EAC gallery', 'eac-components' );
		$this->description   = esc_html__( 'Enhance your WordPress site with the ACF Image Gallery and display it with Elementor', 'eac-components' );
		...
		<div class="<?php echo esc_attr( $this->add_class( 'container' ) ); ?>">
			<div class="<?php echo esc_attr( $this->add_class( 'attachments' ) ); ?> <?php echo esc_attr( $this->add_class( 'attachments' ) ); ?>-<?php echo esc_attr( $field['key'] ); ?>">
				<?php if ( ! empty( $attachments ) ) :
					foreach ( $attachments as $attachment ) :
						...
						if ( ! empty( $attachment['metadata'] ) && ! empty( $attachment['metadata']['medium']['url'] ) ) :
							$media_url = $attachment['metadata']['medium']['url'];
						elseif ( ! empty( $attachment['metadata'] ) && ! empty( $attachment['metadata']['thumbnail']['url'] ) ) :
							$media_url = $attachment['metadata']['thumbnail']['url'];
						endif;

						if ( '' === $media_url ) :
							$media_url   = EAC_PLUGIN_URL . 'includes/acf/fields/gallery/assets/images/field-preview-gallery.png';
			...
			<div class="<?php echo esc_attr( $this->add_class( 'button-bottom' ) ); ?>">
				<button type='button' class="button button-primary <?php echo esc_attr( $this->add_class( 'add-media' ) ); ?>">
					<span class='dashicons dashicons-plus-alt2' aria-hidden='true'></span> <?php esc_html_e( 'Add images', 'eac-components' ); ?>
				</button>
				<button type='button' class="button button-primary <?php echo esc_attr( $this->add_class( 'remove-all-media' ) ); ?>">
					<span class='dashicons dashicons-minus' aria-hidden='true'></span> <?php esc_html_e( 'Remove all images', 'eac-components' ); ?>
				</button>
				<div class="<?php echo esc_attr( $this->add_class( 'message-media' ) ); ?>"><?php esc_html_e( "Don't forget to save your changes", 'eac-components' ); ?></div>
			</div>
		</div>
		<?php
	}
	...
	public function input_admin_enqueue_scripts(): void {
		...
		// register & include JS
		wp_register_script( 'eac-acf-gallery', EAC_Plugin::instance()->get_script_url( 'includes/acf/fields/gallery/assets/js/field-gallery' ), array(), EAC_PLUGIN_VERSION, true );
		...
		wp_register_style( 'eac-acf-gallery', EAC_Plugin::instance()->get_style_url( 'includes/acf/fields/gallery/assets/css/field-gallery' ), array(), EAC_PLUGIN_VERSION );
...
add_action('init', function () {
	acf_register_field_type( 'EACCustomWidgets\Includes\Acf\Fields\Gallery\Eac_Acf_Gallery_Field' );
}, 99 );
