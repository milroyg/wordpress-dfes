 ...
 * PageLayoutSettings
 */
final class Page_Layout_Settings {
	...
	public function get_current_screen_action() {
		...
		if ( is_object( $screen ) ) {
			error_log( '=====>' . wp_json_encode( $screen ) ); // phpcs:ignore
		}
		remove_action( 'current_screen', array( $this, 'get_current_screen_action' ) );
	}
	/**
	 * Enregistrement de deux post meta
	 *
	...
	public function enqueue_scripts() {
		...
		if ( is_object( $screen ) && ! in_array( $screen->id, array( 'widgets', 'site-editor', 'customize', 'profile' ), true ) ) {
			wp_register_script( 'etb-meta-sidebar', EAC_PLUGIN_URL . 'includes/templates-lib/assets/js/meta-sidebar.min.js', array( 'wp-blocks', 'wp-element', 'wp-components' ), '2.1.0', true );
	...
	private function list_header_templates() {
		...
		$headers = get_posts(
			array(
				...
				'post_status'            => 'publish',
				'meta_key'               => Source_Local::TYPE_META_KEY,
