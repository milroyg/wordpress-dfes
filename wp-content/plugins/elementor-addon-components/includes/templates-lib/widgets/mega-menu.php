...
class Mega_Menu_Widget extends Widget_Base {
	...
	protected function render_megamenu() {
		...
		$args = array(
			...
			'walker'          => new Class_Menu_Walker( $settings['mn_content_orientation'], false ),
		);
		...
		$this->add_render_attribute( 'nav_menu', 'data-enable-fixed', esc_attr( $sticky ) );
		$this->add_render_attribute( 'nav_menu', 'data-menu-cache', false );
		$this->add_render_attribute( 'nav_menu', 'data-item-reverted', isset( $settings['mn_content_item_revert'] ) && 'yes' === $settings['mn_content_item_revert'] ? true : false );

		/** Ajout du filtre avant de créer le menu */
		if ( class_exists( 'WooCommerce' ) && ! Eac_Woo_Filters::instance()->is_catalog() && ! is_cart() && ! is_checkout() && 'yes' === $settings['mn_content_add_menu_cart'] ) {
			Eac_Woo_Filters::instance()->set_mini_cart( boolval( 1 ) );
			$this->add_render_attribute( 'nav_menu', 'data-mini-cart', true );
			add_filter( "wp_nav_menu_{$current_menu_obj->slug}_items", array( $this, 'add_custom_cart' ), 10, 2 );
		} else {
			$this->add_render_attribute( 'nav_menu', 'data-mini-cart', false );
		} ?>
		<style>
		...
		<?php
		$this->add_render_attribute( 'button_open', 'id', esc_attr( $toggle_open ) );
		$this->add_render_attribute( 'button_open', 'type', 'button' );
		$this->add_render_attribute( 'button_open', 'class', 'mega-menu_nav-toggle elementor-clickable mega-menu_flyout-open' );
		...
		$this->add_render_attribute( 'button_close', 'class', 'mega-menu_nav-toggle elementor-clickable mega-menu_flyout-close' );
