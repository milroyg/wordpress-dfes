...
final class Manager {
	...
	public function render_default_header_content() {
		...
		if ( $header_template_id ) {
			...
			echo Plugin::$instance->frontend->get_builder_content_for_display( $header_template_id ); // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
	...
	public function render_default_site_footer() {
		...
		if ( $footer_template_id ) {
			...
			echo Plugin::$instance->frontend->get_builder_content_for_display( $footer_template_id ); // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '</div>';
		}
	}
		...
		if ( ! empty( $type ) && in_array( $type, array( SiteHeader::TYPE, SiteFooter::TYPE ), true ) ) {
			...
			if ( isset( $settings['show_on'] ) ) {
				switch ( $settings['show_on'] ) {
					case 'global':
						update_option( $key_prefix . 'global', $post_name, false );
						break;
					case 'blog':
					case 'index':
						update_option( $key_prefix . 'blog', $post_name, false );
						break;
					case 'front':
						update_option( $key_prefix . 'front', $post_name, false );
						break;
					case 'search':
						update_option( $key_prefix . 'search', $post_name, false );
						break;
					case 'err404':
						update_option( $key_prefix . 'err404', $post_name, false );
						break;
					case 'wc_shop':
						update_option( $key_prefix . 'wc_shop', $post_name, false );
						break;
					case 'privacy':
						update_option( $key_prefix . 'privacy', $post_name, false );
						break;
					case 'singular':
						if ( ! empty( $settings['singular_pages'] ) ) {
							foreach ( $settings['singular_pages'] as $page_type ) {
								update_option( $key_prefix . 'singular_' . $page_type, $post_name, false );
							}
						} else {
							update_option( $key_prefix . 'singular', $post_name, false );
						}
						break;
					case 'archive':
						if ( ! empty( $settings['archive_pages'] ) ) {
							foreach ( $settings['archive_pages'] as $page_type ) {
								update_option( $key_prefix . 'archive_' . $page_type, $post_name, false );
							}
						} else {
							update_option( $key_prefix . 'archive', $post_name, false );
						}
						break;
					case 'custom':
						if ( ! empty( $settings['singular_pages'] ) ) {
