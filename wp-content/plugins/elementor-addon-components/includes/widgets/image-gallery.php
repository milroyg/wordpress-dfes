<?php
/**
 * Class: Image_Gallery_Widget
 * Name: Galerie d'Images
 * Slug: eac-addon-image-galerie
 *
 * Description: affiche des images dans différents modes
...
class Image_Gallery_Widget extends Widget_Base {
		...
		$this->start_controls_section(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'label_block' => true,
					'selectors'   => array(
						'{{WRAPPER}} .image-galerie__overlay' => 'justify-content: {{VALUE}};',
					),
					'conditions'  => array(
						'relation' => 'or',
						'terms'    => array(
			...
			$this->add_responsive_control(
				...
				array(
					...
					'toggle'    => false,
					'selectors' => array(
						'{{WRAPPER}} .image-galerie__overlay' => 'align-items: {{VALUE}};',
						'{{WRAPPER}} .image-galerie__overlay .image-galerie__description-wrapper' => 'text-align: {{VALUE}};',
					),
					'condition' => array( 'ig_layout_type!' => 'justify' ),
				)
			);
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				array(
					...
					'global'    => array( 'default' => Global_Colors::COLOR_SECONDARY ),
					'selectors' => array(
						'{{WRAPPER}} .ig-filters__wrapper .ig-filters__item.ig-active:after' => 'border-block-end: 3px solid {{VALUE}};',
	...
	protected function render_galerie() {
		...
		foreach ( $settings['ig_image_list'] as $index => $item ) {
			...
			if ( $has_filters && ! empty( $item['ig_item_filter'] ) ) {
				...
				$this->add_render_attribute( 'gallery_item', 'class', array( 'image-galerie__item', implode( ' ', $sanized ) ) );
			} elseif ( ! $has_filters || empty( $item['ig_item_filter'] ) ) {
				if ( 'justify' === $layout_type ) {
	...
	protected function get_settings_json() {
		...
		$module_settings = array(
			...
			'data_animate'             => 'yes' === $settings['ig_enable_animation'] ? true : false,
			'data_lazy'                => 'yes' === $settings['ig_image_lazy'] ? true : false,
		);
