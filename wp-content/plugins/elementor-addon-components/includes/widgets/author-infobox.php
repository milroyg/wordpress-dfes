...
class Author_Infobox_Widget extends Widget_Base {
	...
	protected function render_infobox() {
		...
		if ( ! empty( get_the_author_meta( 'display_name', $authordata->ID ) ) ) {
			$name = $open_name . $prefix . esc_html( ucfirst( get_the_author_meta( 'display_name', $authordata->ID ) ) ) . $close_name;
		}
		...
		if ( $has_role ) {
			...
			if ( ! empty( $user_info->roles ) && is_array( $user_info->roles ) ) {
				...
				$role  = '<' . $title_tag . ' class="author-infobox_role-content">' . ucfirst( $title ) . '</' . $title_tag . '>';
			}
		}
		...
		<div <?php $this->print_render_attribute_string( 'content_wrapper' ); ?>>
			<?php if ( ! empty( $avatar_url ) ) : ?>
				<div class='author-infobox_image'>
					<img class='avatar photo' src="<?php echo esc_url( $avatar_url ); ?>" alt="<?php echo 'Avatar of ' . esc_attr( ucfirst( $author_nicename ) ); ?>"  loading='lazy' width='150' height='150' />
				</div>
			<?php endif; ?>
			...
			<div class='author-infobox_wrapper-info'>
				<div class='author-infobox_info-content'>
					...
					<?php endif; ?>
					<?php if ( $has_button ) :
						$label = ! empty( $settings['aib_relation_more_label'] ) ? $settings['aib_relation_more_label'] : esc_html__( 'Voir les archives', 'eac-components' );
						...
						$this->add_render_attribute( 'button_readmore', 'aria-label', sprintf( '%s of %s', esc_attr( $label ), esc_attr( $author_nicename ) ) );
