...
class Eac_Custom_Attributes {
	...
	public function __construct() {
		add_action( 'elementor/element/after_section_end', array( $this, 'eac_attributes_controls_section' ), 10, 2 );
	 ...
	 * @param Array        $args
	 */
	public function eac_attributes_controls_section( $element, $section_id ) {

		if ( ! $element instanceof Element_Base ) {
			return;
		}

		// Le control PRO existe
		if ( 'section_custom_attributes_pro' === $section_id ) {
			...
			if ( ! is_wp_error( $section_pro ) && in_array( $section_pro['tab'], array( 'advanced' ), true ) ) {
				\Elementor\Plugin::$instance->controls_manager->remove_control_from_stack( $element->get_unique_name(), array( $section_id, 'custom_attributes_pro' ) );
