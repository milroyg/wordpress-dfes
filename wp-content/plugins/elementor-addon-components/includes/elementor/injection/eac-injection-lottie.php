...
class Eac_Injection_Lottie {
	...
	public function inject_section( $element, $section_id ) {
		...
		if ( 'section_effects' === $section_id && in_array( $element->get_name(), $this->target_elements, true ) ) {
			...
			$element->start_controls_section(
				...
				$element->add_control(
					'eac_element_lottie_is_json_enable',
					array(
						'label'        => 'JSON URL',
				...
				$element->add_control(
					...
					array(
						...
						'condition'   => array(
							...
							'eac_element_lottie_is_json_enable' => 'true',
						),
					)
				);
				...
				$element->add_control(
					...
					array(
						...
						'condition'   => array(
							'eac_element_lottie'                => 'yes',
							'eac_element_lottie_source'         => 'url',
							'eac_element_lottie_is_json_enable' => 'false',
						),
					)
				);
	...
	public function render_lottie( $element ) {
		...
		if ( isset( $settings['eac_element_lottie'] ) && 'yes' === $settings['eac_element_lottie'] ) {
			...
			} elseif ( 'true' === $settings['eac_element_lottie_is_json_enable'] && 'url' === $settings['eac_element_lottie_source'] && ! empty( $settings['eac_element_lottie_media_url']['url'] ) ) {
	...
	public function print_template( $template, $element ) {
		...
		if(settings.eac_element_lottie && 'yes' === settings.eac_element_lottie) {
			...
			if ('file' === settings.eac_element_lottie_source) {
				url = settings.eac_element_lottie_media_file;
			} else if ('true' === settings.eac_element_lottie_is_json_enable && 'url' === settings.eac_element_lottie_source && '' !== settings.eac_element_lottie_media_url.url ) {
				url = settings.eac_element_lottie_media_url.url;
			}
		...
		#>
			...
			<div class="lottie-anim_wrapper lottie-anim_wrapper-bg" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; min-block-size: 50px;"
			data-src="{{ url }}"
