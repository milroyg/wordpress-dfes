...
function dfes_admin_enqueue_scripts($hook) {
  ...
  wp_enqueue_script('datatables-js', 'https://cdn.datatables.net/1.13.7/js/jquery.dataTables.min.js', ['jquery'], NULL, TRUE);
  wp_enqueue_script('datatables-bootstrap5-js', 'https://cdn.datatables.net/1.13.7/js/dataTables.bootstrap5.min.js', [
    'datatables-js',
    'bootstrap-js',
  ], NULL, TRUE);
          ...
          foreach ($station_parts as $part) {
            if (strtolower($part) === 'all') {
              ...
              $stations_arr = array_merge(
                $stations_arr,
                array_diff(array_keys($stations_list), [
                  'Press',
                  'TestStation',
                  'Reinforcement',
                ])
  ...
  <div class="container-fluid">
    ...
    <?php if (isset($_GET['success'])): ?>
      <div class="alert alert-success">
        ✅ <?php echo esc_html($_GET['success']); ?> successfully
      </div>
    <?php endif; ?>
    ...
    <div class="card border-success mb-3">
      <div
        class="card-header"><?php echo $edit_contact ? 'Edit Contact' : 'Add Contact'; ?></div>
      <div class="card-body">
        <form method="POST" action="<?php echo admin_url('admin-post.php'); ?>">
          ...
          <div class="mb-3"><label>Name</label><input type="text" name="name"
                                                      ...
                                                      required></div>
          <div class="mb-3"><label>Phone</label><input type="text"
                                                       name="phone_number"
                                                       class="form-control"
    ...
    <form method="POST" action="<?php echo admin_url('admin-post.php'); ?>"
      ...
      <table id="dfes-contacts-table" class="table table-bordered">
        ...
        <?php foreach ($contacts as $c): ?>
          <tr>
            ...
            <td>
              <a
                href="<?php echo admin_url('admin.php?page=dfes-contacts&edit_contact=' . $c['id']); ?>"
                class="link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">Edit</a>
              <a
                href="<?php echo wp_nonce_url(admin_url('admin-post.php?action=dfes_delete_contact&id=' . $c['id']), 'dfes_delete_contact'); ?>"
      ...
      <button type="submit" class="btn btn-outline-danger">Delete Selected
      </button>
    </form>
    <!-- Import/Export CSV -->
    <div class="mt-3">
      <a
        href="<?php echo esc_url(admin_url('admin.php?page=dfes-contacts&export_csv=1')); ?>"
      ...
      <form method="post" action="<?php echo admin_url('admin-post.php'); ?>"
        ...
        <button type="submit" class="btn btn-outline-primary">Import CSV
        </button>
      </form>
    </div>
    ...
    <?php if (isset($_GET['import_done']) && ($result = get_transient('dfes_csv_import_result'))): ?>
      ...
      <?php if ($result['errors'] > 0 && $result['log_url']): ?>
        <div class="notice notice-warning">
          <p><a href="<?php echo esc_url($result['log_url']); ?>">📄 Download
              Error Log</a></p>
        </div>
      <?php endif; ?>
...
function dfes_settings_admin_page() {
  ...
  $all = (array) get_option('dfes_settings', [
    'active' => '',
    'gateways' => [],
  ]);
    ...
    <form method="post">
      ...
      <table class="form-table">
        <tr>
          ...
          <td><input type="text" name="gateway_name"
                     value="<?php echo esc_attr($edit_data['name']); ?>"
                     class="regular-text" required></td>
        </tr>
        <tr>
          <th><label>Base URL</label></th>
          <td><input type="text" name="base_url"
                     value="<?php echo esc_attr($edit_data['base_url']); ?>"
                     class="regular-text" required></td>
        </tr>
      </table>
      ...
      <div id="gateway-params">
        <?php if (!empty($edit_data['params'])): ?>
          <?php foreach ($edit_data['params'] as $param): ?>
            <div class="param" style="margin-bottom:6px;">
              <input type="text" name="params[keys][]"
                     value="<?php echo esc_attr($param['key']); ?>"
                     placeholder="param_name" class="regular-text"
                     style="width:25%;">
              <input type="text" name="params[values][]"
                     value="<?php echo esc_attr($param['value']); ?>"
                     placeholder="param_value (use {mobile}, {message})"
                     class="regular-text"
                     style="width:50%;">
        ...
        <?php else: ?>
          <div class="param" style="margin-bottom:6px;">
            <input type="text" name="params[keys][]" placeholder="param_name"
                   class="regular-text"
                   style="width:25%;">
            <input type="text" name="params[values][]"
                   placeholder="param_value (use {mobile}, {message})"
      ...
      <p>
        <button type="button" class="button" onclick="addParam()">+ Add Param
        </button>
      </p>
      <p>
        <button type="submit" name="dfes_save_gateway"
                class="button button-primary">Save Gateway
        </button>
      </p>
    </form>
            ...
            <td>
              <?php if ($all['active'] !== $id): ?>
                <form method="post" style="display:inline">
                  ...
                  <input type="hidden" name="activate_gateway"
              ...
              <form method="post" style="display:inline">
                <input type="hidden" name="edit_gateway"
                       value="<?php echo esc_attr($id); ?>">
                <button type="submit" name="dfes_edit_gateway"
              ...
              <form method="post" style="display:inline"
                ...
                <input type="hidden" name="delete_gateway"
                       value="<?php echo esc_attr($id); ?>">
                <button type="submit" name="dfes_delete_gateway"
