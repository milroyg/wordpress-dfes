...
if ( ! class_exists( 'Posimyth_Deactivation_Survey' ) ) {
	...
	class Posimyth_Deactivation_Survey {
		...
		private static function default_reasons(): array {
			$labels = array(
				'no-longer-needed' => __( 'I no longer need the plugin', 'she-header' ),
				'found-better'     => __( 'I found a better plugin', 'she-header' ),
				'not-working'      => __( "It's not working", 'she-header' ),
				'temporary'        => __( "It's a temporary deactivation", 'she-header' ),
				'plugin-conflict'  => __( 'Plugin conflict', 'she-header' ),
				'site-speed'       => __( 'It slowed down my site', 'she-header' ),
				'other'            => __( 'Other', 'she-header' ),
			);
			...
			<div id="<?php echo esc_attr( $uid ); ?>" class="posi-deact-modal posi-deact-p-<?php echo esc_attr( sanitize_html_class( $slug ) ); ?>" style="--posi-accent:<?php echo esc_attr( $accent ); ?>">
				<div class="posi-deact-dialog" role="dialog" aria-modal="true" aria-labelledby="<?php echo esc_attr( $uid ); ?>-title" tabindex="-1">
					<h3 id="<?php echo esc_attr( $uid ); ?>-title">
						...
						<?php esc_html_e( 'Quick Feedback', 'she-header' ); ?>
					</h3>
					<p class="posi-deact-sub">
						<?php
						/* translators: %s: product name */
						echo esc_html( sprintf( __( 'Help us improve %s. Why are you deactivating?', 'she-header' ), $name ) );
						?>
					</p>
					<form id="<?php echo esc_attr( $uid ); ?>-form">
						...
						<textarea name="posi_other_text" rows="3" maxlength="<?php echo (int) self::OTHER_TEXT_MAX; ?>" placeholder="<?php esc_attr_e( 'Please share more details…', 'she-header' ); ?>"></textarea>
						...
						<div class="posi-deact-opts">
							...
							<label class="posi-deact-opt">
								...
								<span><?php esc_html_e( 'I agree to be contacted via email for support with this plugin.', 'she-header' ); ?></span>
							</label>
						</div>
						...
						<div class="posi-deact-footer">
							...
							<button type="button" class="posi-deact-skip"><?php esc_html_e( 'Skip & Deactivate', 'she-header' ); ?></button>
							<button type="submit" class="posi-deact-submit"><?php esc_html_e( 'Submit & Deactivate', 'she-header' ); ?></button>
						</div>

						<?php
						/* translators: %s: product name */
						$consent_default = sprintf( __( 'Submitting shares non-sensitive info so we can improve %s. No personal data.', 'she-header' ), $name );
						/* translators: %s: product name */
						$consent_contact = sprintf( __( 'Submitting shares non-sensitive info so we can improve %s, plus your admin email so we can reply.', 'she-header' ), $name );
						?>
			...
			jQuery(function($){
				...
				$(document).on('click.<?php echo esc_js( $uid ); ?>', 'span.deactivate a', function(e){
					...
					function send(){
						...
						var other  = $other.val() || '';
						$submit.prop('disabled', true).text(<?php echo wp_json_encode( __( 'Submitting…', 'she-header' ) ); ?>);
						$.post(ajaxurl, {
