...
<script type="text/javascript" data-wpacu-own-inline-script="true">
    ...
    <?php if (! defined('WPACU_PRO_PLUGIN_VERSION') && ! is_admin()) {
    ...
    ?>
    document.querySelectorAll('.wpacu-manage-hardcoded-assets-requires-pro-popup').forEach(function(element) {
        element.addEventListener('click', function() {
            window.location.assign('<?php echo esc_js($upgradeToProLink); ?>');
        });
    });
    <?php } ?>

    document.addEventListener('click', function(event) {
        var trigger = event.target.closest('.wpacu-locked-checkbox-trigger, .wpacu-locked-checkbox-overlay');
        var visibleTooltips = document.querySelectorAll('.wpacu-locked-checkbox-overlay .wpacu-tooltip.wpacu-is-click-visible');

        visibleTooltips.forEach(function(tooltip) {
            if (! trigger || ! trigger.closest('.wpacu-locked-checkbox-option').contains(tooltip)) {
                tooltip.classList.remove('wpacu-is-click-visible');
                var previousOverlay = tooltip.closest('.wpacu-locked-checkbox-overlay');
                if (previousOverlay) {
                    previousOverlay.classList.remove('wpacu-has-visible-tooltip');
                }
            }
        });
        ...
        var option = trigger.closest('.wpacu-locked-checkbox-option');
        var tooltip = option ? option.querySelector('.wpacu-locked-checkbox-overlay .wpacu-tooltip') : null;
        ...
        tooltip.classList.toggle('wpacu-is-click-visible');
        var overlay = tooltip.closest('.wpacu-locked-checkbox-overlay');
        if (overlay) {
            overlay.classList.toggle('wpacu-has-visible-tooltip', tooltip.classList.contains('wpacu-is-click-visible'));
        }
    }, true);
</script>
