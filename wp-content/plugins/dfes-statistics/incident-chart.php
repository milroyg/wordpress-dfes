...
function incident_chart_shortcode() {
  ...
  ob_start();
  ?>
  <div class="incident-chart-container" role="region" aria-label="Statistics"
       tabindex="-1">
    <h3>Select Criteria</h3>
    <label for="category-filter" class="screen-reader-text">Select Incident
      Category</label>
    <select id="category-filter" name="category-filter">
      <?php foreach (array_keys($data) as $stat_type): ?>
        <option
          value="<?= esc_attr($stat_type) ?>" <?= $stat_type === $first_category ? 'selected' : '' ?>>
          <?= esc_html($stat_type) ?>
        </option>
      <?php endforeach; ?>
    </select>

    <canvas id="incident-chart" width="800" height="400" role="img"
            aria-label="Incident Chart"></canvas>
  </div>
      ...
      function renderTable() {
        ...
        const rows = [
          ...
          },
          {
            ...
            percentage: '436%',
            arrow: '<span style="color:#900">▲</span>',
            color: ' #990000',
          },
          {
            label: 'Lives Lost (H)',
