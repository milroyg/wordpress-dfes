...
/**
 * Moved the edd-sl-plugin-updater (only the TRP_EDD_SL_Plugin_Updater class) from free version to this package.
 * This way it remains compliant with WordPress.org, since we are not allowed to interfere with the WP updating system.
 * Since TranslatePress - Personal 1.6.7
 */
require_once plugin_dir_path( __FILE__ ) . 'class-edd-sl-plugin-updater.php';
...
if( !class_exists('TRP_Handle_Included_Addons') ){
    class TRP_Handle_Included_Addons{
        ...
        function __construct(){
            ...
            // Revert the free version "TranslatePress AI API Key" wording back to "License" on Pro versions.
            add_filter( 'trp_ai_api_key_labels', array( $this, 'trp_revert_license_labels' ) );

            add_action( 'admin_init', array( $this, 'maybe_activate_tp_free' ) );

            //check the license when WordPress checks for plugin updates
            add_action( 'plugins_loaded', array( $this, 'trp_add_license_update_check' ), 15 );
        ...
        /**
         * Hook the license check into WordPress' plugin update check.
         *
         * Runs after the main plugin has loaded so the 'plugin_updater' component is available.
         *
         *  Since TranslatePress - Personal 1.6.8
         */
        public function trp_add_license_update_check(){
            if ( ! class_exists( 'TRP_Translate_Press' ) )
                return;

            $plugin_updater = TRP_Translate_Press::get_trp_instance()->get_component( 'plugin_updater' );
            if ( $plugin_updater ) {
                if (method_exists( $plugin_updater, 'check_license' ) ) {
                    add_filter('pre_set_site_transient_update_plugins', array($plugin_updater, 'check_license'));
                }
                if (method_exists( $plugin_updater, 'check_tp_api_key' ) ) {
                    add_filter('pre_set_site_transient_update_plugins', array($plugin_updater, 'check_tp_api_key'));
                }
            }
        }

        /**
         * Revert the License page / tab / onboarding labels to the classic "License" wording.
         *
         * The free version (translatepress-multilingual) uses "TranslatePress AI API Key"
         * wording by default. Pro versions load this file and restore the "License" wording.
         *
         * @param array $labels Default (free) labels.
         * @return array
         */
        public function trp_revert_license_labels( $labels ){
            $labels['tab']                        = __( 'License', 'translatepress-multilingual' );
            $labels['status_valid']               = __( 'Your License Key is valid.', 'translatepress-multilingual' );
            $labels['status_invalid']             = __( 'Your License Key is invalid.', 'translatepress-multilingual' );
            $labels['status_expired']             = __( 'Your License has expired.', 'translatepress-multilingual' );
            $labels['activate_button']            = __( 'Activate License', 'translatepress-multilingual' );
            $labels['deactivate_button']          = __( 'Deactivate License', 'translatepress-multilingual' );
            $labels['add_heading']                = __( 'Add a license key', 'translatepress-multilingual' );
            $labels['field_label']                = __( 'License Key', 'translatepress-multilingual' );
            $labels['manage']                     = __( 'Manage your license in your %1$s.', 'translatepress-multilingual' );
            $labels['dont_have_heading']          = __( 'Don’t have a TranslatePress AI License Key?', 'translatepress-multilingual' );
            $labels['get_free_button']            = __( 'Get a free License Today', 'translatepress-multilingual' );
            $labels['onboarding_heading']         = __( 'Add your License Key', 'translatepress-multilingual' );
            $labels['onboarding_subheading']      = __( 'Add your License Key to unlock all premium features. Find the License Key in your', 'translatepress-multilingual' );
            $labels['onboarding_valid']           = __( 'Your license is valid and active.', 'translatepress-multilingual' );
            $labels['onboarding_field_label']     = __( 'License Key', 'translatepress-multilingual' );
            $labels['onboarding_activate_button'] = __( 'Activate License', 'translatepress-multilingual' );

            $labels['submenu_page_title']         = __( 'TranslatePress License', 'translatepress-multilingual' );

            $labels['no_active_detected']         = __( 'No Active License Detected for this website.', 'translatepress-multilingual' );
            $labels['need_key_free_account']      = __( 'In order to enable Automatic Translation using TranslatePress AI, you need a license key by creating a free account.', 'translatepress-multilingual' );
            $labels['enter_key']                  = __( 'Enter your license key', 'translatepress-multilingual' );

            $labels['enter_key_from']             = __( 'In order to enable Automatic Translation using TranslatePress AI, please enter your license key from', 'translatepress-multilingual' );
            $labels['ai_field_label']             = __( 'License Key', 'translatepress-multilingual' );
            $labels['ai_activate_button']         = __( 'Activate License', 'translatepress-multilingual' );
            $labels['no_active_detected_ai']      = __( 'No Active License Detected for this website.', 'translatepress-multilingual' );
            $labels['get_free_ai_heading']        = __( 'Get Your Free TranslatePress AI License', 'translatepress-multilingual' );
            $labels['generate_button']            = __( 'Generate License', 'translatepress-multilingual' );
            $labels['valid_product']              = __( 'You have a valid %s <strong>license</strong>.', 'translatepress-multilingual' );
            $labels['manage_quota']               = __( 'Manage your license & quota on the %s', 'translatepress-multilingual' );

            $labels['get_free_ai_button']         = __( 'Get a Free AI License', 'translatepress-multilingual' );

            $labels['already_on_free']            = __( 'This website is already activated under a free license. Each website can only use one free license.', 'translatepress-multilingual' );
            $labels['already_on_free_upgrade']    = __( 'This website is already activated under a free license. Each website can only use one free license. Please upgrade to a premium plan for more TranslatePress AI words from %1$s your account %2$s.', 'translatepress-multilingual' );

            $labels['debug_checking']             = __( 'Debug Data for License Checking', 'translatepress-multilingual' );
            $labels['debug_activation']           = __( 'Debug Data for License Activation', 'translatepress-multilingual' );
...
function trp_personal_update(){
    ...
    if (class_exists('TRP_EDD_SL_Plugin_Updater') && class_exists('TRP_Translate_Press') ) {
        ...
        $edd_updater = new TRP_EDD_SL_Plugin_Updater(TRP_PER_STORE_URL, __FILE__, array(
                'version' => '1.6.9',        // current version number
