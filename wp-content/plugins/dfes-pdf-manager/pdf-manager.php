...
function tam_enqueue_frontend_styles() {
    if ($GLOBALS['tam_pdf_shortcode_used']) {
        wp_enqueue_style('tam-pdf-style', plugin_dir_url(__FILE__) . 'assets/style.css', array(), '1.0');
    }
}
...
function tam_add_category_shortcode_field($term) {
    ...
    <tr class="form-field">
        ...
        <td>
            <input type="text" class="widefat" value='[pdf category="<?php echo esc_attr($category_slug); ?>"]' readonly onclick="this.select();" />
            <br />
            <input type="text" class="widefat" value='[pdf_<?php echo esc_attr($category_slug); ?>]' readonly onclick="this.select();" />
