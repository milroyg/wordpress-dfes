...
function tam_register_post_type() {
  register_post_type('tam_pdf', [
    ...
    'taxonomies' => ['tam_category'],
  ]);

  register_taxonomy('tam_category', 'tam_pdf', [
    'label' => __('Categories', 'tam'),
    'rewrite' => ['slug' => 'tam-category'],
    'hierarchical' => TRUE,
  ]);
}

/** 2. Add Category Column in Admin List */
...
function tam_save_meta($post_id) {
  if (isset($_POST['tam_pdf_link'])) {
    update_post_meta($post_id, 'tam_pdf_link', esc_url($_POST['tam_pdf_link']));
  }
  if (isset($_POST['tam_title_en'])) {
    update_post_meta($post_id, 'tam_title_en', sanitize_text_field($_POST['tam_title_en']));
  }
  if (isset($_POST['tam_title_mr'])) {
    update_post_meta($post_id, 'tam_title_mr', sanitize_text_field($_POST['tam_title_mr']));
  }
  if (isset($_POST['tam_title_kok'])) {
    update_post_meta($post_id, 'tam_title_kok', sanitize_text_field($_POST['tam_title_kok']));
  }
}
...
function tam_enqueue_admin_scripts($hook) {
  global $post_type;
  if (($hook === 'post.php' || $hook === 'post-new.php') && $post_type === 'tam_pdf') {
    wp_enqueue_media();
  }
}
...
/** 6. Auto-generate Slug from PDF Name */
function tam_update_post_slug_from_pdf($post_id) {
  if (get_post_type($post_id) !== 'tam_pdf') {
    return;
  }
...
function tam_display_pdfs($atts) {
  ...
  while ($query->have_posts()) {
    ...
    if ($pdf_link) {
      ...
      $pdf_size = file_exists($pdf_path) ? tam_format_file_size(filesize($pdf_path)) : 'Unknown size';
      ...
      $output .= '<li class="tam-pdf-item">
                ...
                <span class="tam-date" style="color:#000">' . esc_html($date_added) . ' - </span>
                <a href="' . esc_url($pdf_link) . '" target="_blank" rel="noopener noreferrer">' . esc_html($display_title) . '</a>
...
/** 8. Load CSS if shortcode used */
function tam_enqueue_frontend_styles() {
  if ($GLOBALS['tam_pdf_shortcode_used']) {
    wp_enqueue_style('tam-pdf-style', plugin_dir_url(__FILE__) . 'assets/style.css', [], '1.0');
  }
}
  ...
  if (!empty($categories) && !is_wp_error($categories)) {
    foreach ($categories as $category) {
      add_shortcode('pdf_' . $category->slug, function() use ($category) {
        return tam_display_pdfs(['category' => $category->slug]);
      });
    }
  }
}

/** 11. Show Shortcodes on Category Edit Page */
  ...
  <tr class="form-field">
    ...
    <td>
      <input type="text" class="widefat"
             value='[pdf category="<?php echo esc_attr($category_slug); ?>"]'
             readonly onclick="this.select();"/>
      <br/>
      <input type="text" class="widefat"
             value='[pdf_<?php echo esc_attr($category_slug); ?>]' readonly
             onclick="this.select();"/>
      <p class="description">Use either shortcode above to display PDFs from
        this category.</p>
    </td>
  </tr>
  <?php
...
function tam_meta_callback($post) {
  ...
  <p><a href="<?php echo esc_url($pdf_link); ?>" target="_blank"
        id="tam_pdf_preview" <?php echo empty($pdf_link) ? 'style="display:none;"' : ''; ?>>View
      PDF</a></p>
  <p>
    <label for="tam_pdf_link"><?php _e('PDF Link', 'tam'); ?></label><br/>
    <input type="text" id="tam_pdf_link" name="tam_pdf_link"
           value="<?php echo esc_attr($pdf_link); ?>" class="widefat"/>
  </p>
  <script>
      ...
      $('.tam-upload-pdf').click(function (e) {
        ...
        mediaUploader.on('select', function () {
          ...
          if (!titleField.val().trim()) {
            titleField.val(fileName).trigger('input');
          }
        });
        mediaUploader.open();
      });
...
function tam_add_meta_boxes() {
  add_meta_box('tam_pdf_meta_box', __('PDF Details', 'tam'), 'tam_meta_callback', 'tam_pdf', 'normal', 'high');
}

/**
 * -------------------------------
 * HOOKS (all together for clarity)
 * -------------------------------
 */
...
add_action('wp_footer', 'tam_enqueue_frontend_styles');
